                        ...
                        <div class="bg-white rounded-lg px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="">
                                <div class="mt-3 sm:mt-0 sm:ml-4 sm:text-left">
                                    <header>
                                        <div>
                                            <h1 class="text-3xl bold">Изменение пользователя</h1>
                                        </div>
                                    </header>
...
<!--                                            </div>-->
                                    ...
                                    <div class="mt-2">
                                        ...
                                        <div class="mt-1">
                                            <select v-if="lessons" name="lesson[]"  id="lesson" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md" multiple>
                                                <option value="null" :selected="!teacher.lessons">Не выбрано</option>
                                                <option v-bind:value="lesson.id" v-for="lesson in lessons" :selected="lessonSelect(lesson)">{{ lesson.name }}</option>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-2">
                                        ...
                                        <div class="mt-1" v-if="teacher">
                                            <input type="text" name="type" id="type" v-bind:value="teacher.type" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Должность">
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-2">
                                        ...
                                        <div class="mt-1" v-if="classes">
                                            <select type="text" name="classroom_teacher" id="classroom_teacher" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                                                <option value="null" :selected="teacher.class_id">Нет</option>
                                                <option v-bind:value="cl.id" v-for="cl in classes" :selected="cl.id === teacher.class_id">{{ cl.number }}{{ cl.letter }}</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            getChanges: function ()
            {
                ...
                for (let option of document.getElementById('lesson').options)
                {
                    if (option.selected) {
                        ...
                        this.changedTeacher.lessons[i] = {'id': option.value};
            ...
            {
                await axios.get('/teacher/get/' + this.id)
                    .then(response => {
            ...
            lessonSelect: function (lesson)
            {
                ...
                let num = this.teacher.lessons;

                let lessons = Object.keys(num).map((key) => num[key]);
