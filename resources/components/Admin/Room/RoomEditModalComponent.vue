                            ...
                            <div>
                                ...
                                <div class="">
                                    ...
                                    <div class="mt-3 sm:mt-0 sm:ml-4 sm:text-left">
                                        <header>
                                            <div>
                                                <h1 class="text-3xl bold">Изменение кабинета</h1>
                                            </div>
                                        </header>
                                        ...
                                        <div class="mt-2">
                                            ...
                                            <div class="mt-1">
                                                <input type="text" v-bind:value="room.name" name="room" id="editRoom" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Название">
                                            </div>
                                        </div>
                                        ...
                                        <div class="mt-2">
                                            ...
                                            <div class="mt-1">
                                                <select  name="lesson" id="editClass" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Название">
                                                    <option v-for="cl in classes" v-bind:value="cl.id" :selected="cl.id === room.class_id">{{ cl.number + cl.letter }}</option>
                                                </select>
                                            </div>
                                        </div>
...
export default {
    ...
    methods: {
        sendRoom: function (e) {
            ...
            axios.patch('/room/' + this.id, this.updatedRoom)
                .then(() => {
        ...
        getChanges: function () {
          this.updatedRoom.name = document.querySelector('#editRoom').value;
          this.updatedRoom.class_id = document.querySelector('#editClass').value;
        },

        getData: async function () {
            this.classes = (await axios.get('/classes/get')).data;

            this.room = (await  axios.get('/room/get/' + this.id)).data
        },
