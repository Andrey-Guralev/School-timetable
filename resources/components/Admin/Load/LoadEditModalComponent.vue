        ...
        <div class="fixed z-40 inset-0 overflow-y-auto">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <transition enter-active-class="ease-out duration-300" enter-class="opacity-0" enter-to-class="opacity-100" leave-active-class="ease-in duration-200" leave-class="opacity-100" leave-to-class="opacity-0">
                    <div v-if="open" v-on:click="closeModal" class="fixed inset-0 transition-opacity"
                         aria-hidden="true">
                    ...
                    <div v-if="open" class="lg:w-1/2 inline-block align-bottom bg-white rounded-lg text-left shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                        ...
                        <div class="bg-white rounded-lg px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div>
                                ...
                                <div class="">
                                    ...
                                    <div class="mt-3 sm:mt-0 sm:ml-4 sm:text-left">
                                        <header>
                                            <div>
                                                <h1 class="text-3xl bold">Изменение нагрузки</h1>
                                            </div>
                                        </header>
                                        ...
                                        <div class="mt-2">
                                            ...
                                            <div class="mt-1">
                                                <select name="lesson" id="class" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                                                    <option v-for="cl in classes" v-bind:value="cl.id" :selected="cl.id === load.class_id">
                                                        {{ cl.number + cl.letter }}
                                                    </option>
                                                </select>
                                            </div>
                                        ...
                                        <div class="mt-2">
                                            ...
                                            <div class="mt-1">
                                                <select  name="lesson" id="lesson" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                                                    <option v-for="lesson in lessons" :selected="lesson.id === load.lesson_id" v-bind:value="lesson.id" >
                                                        {{ lesson.name }}
                                                    </option>
                                                </select>
                                            </div>
                            ...
                            <div class="bg-gray-50  px-4 py-3 sm:px-6 flex justify-end">
                                <button v-on:click="sendLoad" type="submit" class="w-1/3 inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-600 text-base font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:ml-3 sm:w-auto sm:text-sm">
                                    Сохранить
                                </button>
...
export default {
    ...
    methods: {
        ...
        sendLoad: function (e) {
            ...
            let load = {
                lesson_id: document.querySelector('#lesson').value,
                class_id: document.querySelector('#class').value,
                teacher_id: document.querySelector('#teacher').value,
            };

            axios.patch('/load/' + this.id, load)
                .then(response => {
        ...
        getData: async function () {
            this.load = (await axios.get('/load/get/' + this.id)).data;
            if (this.lessons === Object) this.lessons = (await axios.get('/lesson/get')).data;
            if (this.teachers === Object) this.teachers = (await axios.get('/teacher/get')).data;
            if (this.classes === Object) this.classes = (await axios.get('/classes/get')).data;
        },
