    ...
    <x-container>
        ...
        <div class="ml-8 flex">
            @foreach($classes as $class)
                <button type="button" class="class-button id-{{ $class->id }} text-blue-600 mr-4" data-id="{{ $class->id }}" data-number="{{ $class->number }}" data-letter="{{ $class->letter }}" data-url="{{ route('updateClasses') }}">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                <form>
                    ...
                    <div class="mt-5 sm:mt-6 flex justify-between">
                        <button type="button" id="close-button" class="close w-5/12 inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:text-sm">
                            Выйти
                        </button>
